...
class HTTPRequestSecureLoader: NSObject, WebContentsSecureLoader, WKScriptMessageHandler {
    ...
    weak var delegate: WebContentsSecureLoaderDelegate?
    ...
    private func prepareRendering(_ contents: WebContents, into config: WKWebViewConfiguration) {
        ...
        metaWidth.content = "width=device-width, initial-scale=1.0, shrink-to-fit=no";
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if let sanitized = dict["clearBody"] as? String {
            ...
            document.addEventListener('readystatechange', function(event) {
                window.webkit.messageHandlers.loaded.postMessage({'height': document.body.scrollHeight});
            });
            """
        ...
        if let height = dict["height"] as? Double {
            self.delegate?.webContentsHeightDidUpdate(CGFloat(height))
        }
    }
